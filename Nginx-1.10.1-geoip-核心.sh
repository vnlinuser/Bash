...
#安装必要的组件：
yum install -y gcc gcc-c++ pcre zlib pcre-devel openssl-devel xml2 libxslt-devel gunzip libxslt libxml2 gd gd-devel geoip geoip-devel wget git
...
wget -O ${SRC}/GeoIP.dat.gz  http://geolite.maxmind.com/download/geoip/database/GeoLiteCountry/GeoIP.dat.gz
wget -O ${SRC}/GeoLiteCity.dat.gz  http://geolite.maxmind.com/download/geoip/database/GeoLiteCity.dat.gz

gunzip -q ${SRC}/GeoIP.dat.gz -d ${SRC}/ && echo $?
gunzip -q ${SRC}/GeoLiteCity.dat.gz -d ${SRC}/ && echo $?
...
http {
    ...
    #日志格式配置
    log_format  main  '$remote_addr $realip_remote_addr $remote_user [$time_local] "$request_method $scheme://$host$request_uri $server_protocol" '
                '"$upstream_addr" "$upstream_cache_status"'
                '$status $body_bytes_sent "$http_referer" '
                '"$http_user_agent" $http_x_forwarded_for $request_time $upstream_response_time $geoip_city $geoip_country_code';
    ...
    gzip_types text/plain application/x-javascript text/css text/htm application/xml;	#和http头有关系，加个vary头，给代理服务器用的，有的浏览器支持压缩，有的不支持，
    ...
    #为给定的key设置最大连接数。这里key是addr，我们设置的值是100，也就是说我们允许每一个IP地址最多同时打开有20个连接。
    limit_conn addr 20;
    ...
    #反向代理临时文件存放目录：
    proxy_temp_path /dev/shm/cache_tmp;
    #反向代理缓存文件存放目录：
    ...
    geoip_proxy 39.109.10.33;
    geoip_proxy 39.109.10.129;
    geoip_proxy 103.226.155.133;
    ...
    #GEOIP 城市
    geoip_city /usr/local/src/GeoLiteCity.dat;
...
stream {
    ...
    #代理Redis
    upstream redis {
        server 192.168.11.7:6379 weight=1 max_fails=3 fail_timeout=10s;
    }
    ...
    #代理MYSQL
    upstream mysql {
        server 192.168.11.6:3306 weight=1 max_fails=3 fail_timeout=10s;
    }
...
#创建一个game 虚拟主机配置：
cat << 'eof' > /usr/local/nginx/conf.d/tomcat-game.conf
#负载均衡
upstream  tomcat-game {
    server 192.168.11.6:8080 max_fails=1 fail_timeout=10s  weight=1;
}
...
server {
    ...
    #域名配置：
    server_name renbofun.com www.renbofun.com *.renbofun.com;
    ...
    #Denly IP access：
    #if ($http_x_forwarded_for = "14.161.3.181") {
    #     return 444;
    #}
    ...
    #设置黑名单：
    #include /usr/local/nginx/block.ip;

    #定义访问日志的保存路径
    access_log /data/nginx/logs/game_access.log main;
    ...
    #反向代理配置，使用realip 模块获取客户端IP而不是上级代理IP
    location / {
        ...
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        set_real_ip_from   39.109.10.0/24;
        set_real_ip_from   103.226.155.133;
...
#创建spider 虚拟主机配置：
cat << 'eof' > /usr/local/nginx/conf.d/tomcat-spider.conf
server {
    listen 80;
    server_name  spider.renbofun.com;
   ...
   if ($host ~* "\d+\.\d+\.\d+\.\d+"){
    ...
    #定义访问日志的保存路径
    #access_log /data/nginx/logs/spider_$year-$month-$day.log main;
    access_log /data/nginx/logs/spider_access.log main;
	...
	proxy_set_header Host $host;
        ...
        proxy_send_timeout         240;
        proxy_read_timeout         240;
        proxy_pass http://192.168.11.6:9080;
    }
...
#日志文件，可以是一组 ，用空格隔开
/data/nginx/logs/game_access.log /usr/local/nginx/logs/*.log {
